...
module.exports = function (app, sessionMap, startDEVICE) {
    ...
    const device = (sender) => {
        if (sessionMap.has(parseInt(sender))) {
            return conn = sessionMap.get(parseInt(sender)).sock
        } else {
            return
        }
    }
    ...
    app.post('/send-message', async (req, res) => {
        ...
        if (device(sender)) {
            ...
            res.status(500).json({
                status: false,
                response: 'Koneksi dengan whatsapp terputus. silahkan kembali ke halaman scan QR terlebih dahulu'
            });
        }
    ...
    app.post('/send-media', async (req, res) => {
        ...
        if (device(sender)) {
            ...
            res.end(JSON.stringify({
                status: false,
                message: 'Koneksi dengan whatsapp terputus. silahkan kembali ke halaman scan QR terlebih dahulu'
            }));
        }
    });
